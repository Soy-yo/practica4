...
public class Junction extends SimulatedObject {
  ...
  private Map<Road, Deque<Vehicle>> enteringRoads;
  ...
  public void vehicleIn(Vehicle vehicle) {
    enteringRoads.get(vehicle.getRoad()).offerLast(vehicle);
  }
  ...
  public void advance() {
    ...
    Deque<Vehicle> vehicleList = enteringRoads.get(currentRoadOn);
    if (!vehicleList.isEmpty()) {
      Vehicle vehicle = vehicleList.peekFirst();
      if (vehicle.moveToNextRoad()) {
        vehicleList.pollFirst();
      }
    }
  }
  ...
  public String generateReport(int time) {
    ...
    for (Map.Entry<Road, Deque<Vehicle>> e : enteringRoads.entrySet()) {
